    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          animate={{ y: 0 }}
          exit={{ y: 300 }}
          className="bg-white rounded-t-3xl p-6 w-full max-w-lg space-y-5 max-h-[85vh] overflow-y-auto"
        >
          {/* Header */}
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-[#3d2b2b]">📊 Bilan de la semaine</h2>
              <p className="text-sm text-gray-400">Semaine {currentWeek} {weekData.fruitEmoji}</p>
            </div>
          ...
          <div className="bg-pink-50 rounded-2xl p-4 border border-pink-100">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-semibold text-[#3d2b2b]">⚖️ Poids</span>
              {weightDiff && (
            ...
            {lastWeight ? (
              <p className="text-2xl font-bold text-[#3d2b2b]">{lastWeight.weight} <span className="text-sm font-normal text-gray-400">kg</span></p>
            ) : (
              <p className="text-sm text-gray-400">Aucune pesée cette semaine</p>
            )}
          </div>

          {/* Eau */}
          <div className="bg-purple-50 rounded-2xl p-4 border border-purple-100">
            <span className="text-sm font-semibold text-[#3d2b2b]">💧 Hydratation moyenne</span>
            <p className="text-2xl font-bold text-[#3d2b2b] mt-1">{avgWater} <span className="text-sm font-normal text-gray-400">ml/jour</span></p>
            ...
            <p className="text-xs text-gray-400 mt-1">Objectif : 2000 ml/jour</p>
          </div>

          {/* Symptômes */}
          {uniqueSymptoms.length > 0 && (
            <div className="bg-green-50 rounded-2xl p-4 border border-green-100">
              <span className="text-sm font-semibold text-[#3d2b2b]">😣 Symptômes cette semaine</span>
          ...
          <div className="bg-orange-50 rounded-2xl p-4 border border-orange-100">
            <span className="text-sm font-semibold text-[#3d2b2b]">👶 Mouvements bébé</span>
            <p className="text-2xl font-bold text-[#3d2b2b] mt-1">{kicksThisWeek.length} <span className="text-sm font-normal text-gray-400">sessions cette semaine</span></p>
          </div>

          {/* Prochain RDV */}
          {nextAppt && (
            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100">
              <span className="text-sm font-semibold text-[#3d2b2b]">📅 Prochain rendez-vous</span>
              <p className="font-semibold text-[#3d2b2b] mt-1">{nextAppt.title}</p>
              <p className="text-sm text-gray-400">{format(new Date(nextAppt.date), "d MMMM yyyy", { locale: fr })} à {nextAppt.time}</p>
            </div>
          )}

          {/* Développement bébé */}
          <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-2xl p-4 border border-pink-100">
            <span className="text-sm font-semibold text-[#3d2b2b]">✨ Cette semaine pour bébé</span>
            <p className="text-sm text-gray-600 mt-1 leading-relaxed line-clamp-3">{weekData.babyDevelopment}</p>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
