...
const ADJECTIFS = ['Rayonnante', 'Courageuse', 'Douce', 'Sereine', 'Petillante', 'Epanouie', 'Lumineuse', 'Joyeuse'];
...
const EXAMPLE_POSTS: CommunityPost[] = [
  {
    ...
    type: 'question',
    content: 'Est-ce que quelqu\'un d\'autre ressent des douleurs dans les cotes a 24 semaines ? Mon bebe adore me donner des coups la-haut !',
    week: 24,
  ...
  },
  {
    ...
    type: 'vecu',
    content: 'Aujourd\'hui j\'ai senti mon bebe bouger pour la premiere fois clairement ! A 20 semaines, c\'est comme des petites bulles qui eclatent. Un moment magique',
    week: 20,
  ...
  },
  {
    id: 'example-3',
    authorId: 'user_epa',
    authorName: 'Epanouie Avocat 🥑',
    type: 'conseil',
    content: 'Conseil pour le 3eme trimestre : mettez un oreiller entre les genoux pour dormir sur le cote. Ca change la vie ! La sage-femme me l\'a recommande a 28 semaines.',
    week: 28,
  ...
  },
  {
    id: 'example-4',
    authorId: 'user_ser',
    authorName: 'Sereine Coco 🥥',
    type: 'soutien',
    content: 'Je suis a 30 semaines et j\'ai peur de l\'accouchement. Quelqu\'une peut partager son experience positive ? J\'ai besoin d\'encouragements',
    week: 30,
  ...
  },
  {
    id: 'example-5',
    authorId: 'user_pet',
    authorName: 'Petillante Pasteque 🍉',
    type: 'vecu',
    content: 'Echographie morphologique a 22 semaines : tout est parfait ! Le medecin a dit "bebe parfait" et j\'ai pleure de joie. C\'est le plus beau jour !',
    week: 22,
  ...
  },
  {
    ...
    type: 'conseil',
    content: 'Pour les brulures d\'estomac (semaine 26) : manger en petites quantites plusieurs fois par jour et eviter de se coucher juste apres le repas. Aussi les amandes aident !',
    week: 26,
      ...
      <div className="space-y-3">
        {filteredPosts.length === 0 && (
          <div className="text-center py-10 text-gray-400 text-sm">
            Aucun post pour l&apos;instant. Soyez la premiere a partager !
          </div>
        )}
        {filteredPosts.map(post => {
          const { color } = POST_TYPES[post.type] ?? POST_TYPES.vecu;
          const { label: typeLabel, emoji: typeEmoji } = POST_TYPES[post.type] ?? POST_TYPES.vecu;
          return (
            <motion.div
              key={post.id}
            ...
            >
              ...
              <div className="flex gap-2 pt-1">
                {REACTION_EMOJIS.map(reactionEmoji => {
                  const count = post.reactions[reactionEmoji] ?? 0;
                  const reacted = localReactions[post.id]?.has(reactionEmoji);
                  return (
                    <button
                      key={reactionEmoji}
      ...
      <AnimatePresence>
        {showModal && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                <h2 className="font-bold text-[#3d2b2b]">Partager avec la communaute</h2>
              ...
              <div className="bg-pink-50 rounded-xl p-3 text-xs text-gray-600 flex items-center gap-2">
                ...
                <span>Publie en tant que : <strong>{myPseudo}</strong></span>
              </div>
