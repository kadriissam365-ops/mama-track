    ...
    <div className="max-w-lg mx-auto" style={{ background: "#fdfaf6", minHeight: "100vh" }}>
      ...
      <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-violet-100 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-800">Galerie 🖼️</h1>
            <p className="text-xs text-violet-500">
              {photos.length} photo{photos.length !== 1 ? "s" : ""} — rétrospective
            </p>
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <>
            <p className="text-xs text-gray-400 mb-4 text-center">
              Ton évolution de la semaine {photos[0]?.week} à la semaine {photos[photos.length - 1]?.week}
            </p>
          ...
          >
            ...
            >
              <div className="bg-white p-4 rounded-2xl shadow-2xl">
                ...
                />
                {modal.note && (
                  <p className="mt-3 text-sm text-gray-600">{modal.note}</p>
                )}
